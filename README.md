...
The local application uses only models already loaded in LM Studio and does not download, load, or unload them. The default **Balanced** profile runs Qwen on every page, sends uncertain/corrected/matrix pages plus a 10% audit sample to Gemma, batches all conflicts on a page into one adjudication request, and then performs reasonableness checks. **Maximum accuracy** remains selectable and runs both vision models on every page. Requests are always sequential, never parallel. YOLO and ONNX Runtime are no longer part of the local installer; the web-server edition retains its separate optional YOLO pipeline.

Local working records and restart state are stored under `%LOCALAPPDATA%\FormSight Local` and expired questionnaire data is purged after 30 days. Every series workbook contains `Questionnaires`, `Long_Answers`, `Page_Extracts`, `Conflicts`, `Failed_Jobs`, `QA_Summary`, `Data_Analysis`, `Run_Log`, `Reasonableness`, and `Review_Audit`. Page results and partial series workbooks are atomically checkpointed after every source; a restarted app offers to resume unfinished batches and skips completed pages. Balanced requests have a 120-second ceiling and do not perform nested transport retries; immediate context-limit errors still reduce the output budget automatically. Failed pages retry once at a smaller image size before becoming a visible flag without stopping later inputs.
