...
The repository also contains **FormSight Local**, a separate single-user PySide6 desktop edition. It has no login or web server, automatically detects an already-running LM Studio server, optionally accepts an explicit private-LAN/VPN server address, accepts multiple questionnaire files, and consolidates every PDF carrying the same operator-assigned series label into one Excel workbook. It does not require an API token.

## FormSight Local desktop edition

On the destination Windows 10/11 x64 PC:

1. Install LM Studio 0.4+, start its server with authentication off, and load two Q4 vision models: `qwen/qwen3-vl-8b` as the primary and `google/gemma-3-4b` as the independent verifier. Use loopback when FormSight and LM Studio run on the same PC. For a separate trusted model PC, enable LAN serving and allow the LM Studio port only from the FormSight/VPN client. For a 16 GB RTX 5060 Ti, use an 8192–12288-token context, enable Flash Attention, and confirm that both models are fully GPU-offloaded. Keep KV cache on GPU/Auto if both models still fit; move it to system RAM only when required to avoid model-layer CPU offload.
2. Run `FormSight-Local-Setup.exe`. Python, Node.js, FastAPI, and an API key are not required on the destination PC.
3. Launch **FormSight Local**. Keep **Auto-detect on this PC**, or type the intended private address such as `192.168.1.50:1234` or `gpu-pc:1234` and press Refresh; the five most recent successful servers are remembered. Drag in or select any number of PDF/PNG/JPEG/single-page TIFF questionnaires, select related rows and assign the same **Series label**, choose an output folder, and start the scan. **Automatic one-take** is the default: after Start, grouping, extraction, reasonableness checks, checkpoint exports, and final Excel exports continue without another click. All files with that label are consolidated into `<series-label>_FormSight.xlsx`; each PDF may independently contain one questionnaire or many. Choose **Auto-group, then circle focus areas** to compare matching pages from the first two questionnaires, draw reusable extraction boxes, and send only those regions to the models. Unmarked page types safely retain full-page scanning. Choose **Review groups first** only when manual boundary confirmation is wanted.

Automatic grouping first reads filename ranges such as `001-010`, verifies the implied page cycle against repeated layout fingerprints, and applies a confirmed cycle to matching PDFs under the same series label. Uncertain files are safely skipped in one-take mode while the rest continue. Optional manual review shows questionnaire page thumbnails, expected count, detected cycle, participant ID, confidence, and editable page ranges.

The local application uses only models already loaded in LM Studio and does not download, load, or unload them. Before processing, every selected vision model must pass a real-image plus strict-JSON smoke test. The default **Balanced** profile discovers each series template once, reads compact answer records with Qwen, and sends only low-confidence or ambiguous pages, the first two calibration questionnaires, and a stratified 10% sample to Gemma. **Higher accuracy** uses larger images and a 20% audit. Requests are always sequential, never parallel; Qwen-only is available as an explicit dropdown choice. YOLO and ONNX Runtime are no longer part of the local installer; the web-server edition retains its separate optional YOLO pipeline.

Local working records and restart state are stored under `%LOCALAPPDATA%\FormSight Local` and expired questionnaire data is purged after 30 days. v0.6 backs up the old database before migration and requires unfinished v0.5 batches to restart. Every series workbook retains the prior sheets and adds `Responses`, `Grouping`, `Data_Dictionary`, and `Run_Settings`. Page results are checkpointed in SQLite after every page and the partial workbook is atomically refreshed after every questionnaire. Requests have a 90-second ceiling, permanent capability errors are never retried, malformed JSON is repaired from saved text without resending the page, and only missing answer-region crops receive one targeted repair. The reasonableness model is flag-only: `Final_Value` always remains the immutable scanner value.
...
The repeatable build creates a PyInstaller one-folder application, wraps it as `release\FormSight-Local-Setup.exe`, and creates a transfer-ready `release\FormSight-Local-v0.6.2-Transfer.zip`. The ZIP contains only the installer, a bilingual quick-start guide, and its SHA-256 checksum. If the installer already exists, `Package-FormSight-Transfer.bat` can rebuild the ZIP without recompiling the app. LM Studio remains responsible for GPU inference, so the desktop installer does not bundle model weights, CUDA libraries, or ONNX Runtime.
...
Remote LM Studio servers normally remain restricted to localhost, private LAN addresses, or VPN addresses. A globally routable IP such as `158.132.152.60` is blocked unless the operator explicitly enables **Advanced: allow an explicit public/routable server IP** and accepts the transmission warning. Enter `http://158.132.152.60:1234` for LM Studio's usual port, or include the actual configured port. Plain HTTP does not encrypt questionnaire images; use HTTPS or a VPN where possible and restrict the model-server firewall to the FormSight PC/VPN. FormSight still rejects authentication-enabled servers because the local edition never asks for or stores an API token.

Questionnaire text and images are untrusted data. The model gateway explicitly refuses document-borne instructions, disables tool use, and accepts only schema-validated JSON. The web edition keeps LM Studio on `127.0.0.1`. The local edition accepts loopback, private IP/VPN ranges, and local hostnames by default; an explicit public IP requires the advanced opt-in and a per-session warning. Any remote HTTP selection sends questionnaire images without transport encryption, so the LM Studio port must be restricted by firewall and must not be left openly accessible from the public internet.
